...
// DefaultUpgradeTimeout is the duration before the Upgrader kills the new
// process if no readiness notification was received.
...
func (u *Upgrader) Stop() {
    u.stopOnce.Do(func() {
        ...
        // prevent new upgrade from happending.
        close(u.stopC)
        
        // Make sure exitC is closed if no upgrade waf running.
        u.upgradeSem <- struct{}{}  
        select {
        case <-u.exitC:
        default:
...
// by the OS.
// Hence we make sure that this file can't be garbage collect by referencing
// it from an Upgrader.
